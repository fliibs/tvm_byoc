...
class GraphExecutorCodegen : public backend::MemoizedExprTranslator<std::vector<GraphNodeRef>> {
 public:
  ...
  LoweredOutput Codegen(IRModule mod, relay::Function func, String mod_name) {
    ...
    // // example StorageInfo show
    // for (auto kv : memory_plan_->expr_to_storage_info) {
    //   const StorageInfo& storage_info = kv.second;
    //   // 打印 storage_info 或者执行其他操作
    //   std::cout << "-------StorageInfo: " << storage_info << "\n";
    // }
    ...
    std::ofstream outfile("graph_json.json", std::ios::trunc);
    if (outfile.is_open()) {
